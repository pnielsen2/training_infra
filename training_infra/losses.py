...
def aleatoric_information_loss(model_preds, x):
    counts = x.sum(axis=-1)
    if 'alpha' in model_preds:
        alpha = model_preds['alpha']
    prior_n = alpha.sum(axis=-1)
    obs_n = x.sum(axis=-1)
    loss_tensor = obs_n*torch.digamma(prior_n+obs_n)-(x*torch.digamma(alpha+x)).sum(axis=-1) -torch.lgamma(obs_n+1) + torch.lgamma(x+1).sum(axis=-1)
    return loss_tensor.sum()/counts.sum()

def epistemic_information_loss(model_preds, x):
    counts = x.sum(axis=-1)
    if 'alpha' in model_preds:
        alpha = model_preds['alpha']
    prior_n = alpha.sum(axis=-1)
    ...
    alpha_plus_x = alpha + x
    loss_tensor =  -torch.lgamma(prior_n) + torch.lgamma(post_n) - (torch.lgamma(alpha_plus_x)-torch.lgamma(alpha)).sum(axis=-1) -obs_n*torch.digamma(post_n) + (x*torch.digamma(alpha_plus_x)).sum(axis=-1)
    return loss_tensor.sum()/counts.sum()
...
loss_dict = {'neg_sqrt_loss': neg_sqrt_loss,
             ...
             'aleatoric_information_loss': aleatoric_information_loss,
             'epistemic_information_loss': epistemic_information_loss
             }